        ...
        <div class="row">
            <div class="col-md-8">
                <div class="box">
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table class="table mb-0" id="sub_sub_category_dt">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
            ...
            <div class="col-md-4">
                <div class="box">
                    ...
                    <div class="box-body">
                        <form novalidate="" method="POST" action="{{ route('sub.sub.category.store') }}">
                            @csrf
                            ...
                            <div class="text-xs-right">
                                <input type="submit" class="btn btn-rounded btn-info" value="Add Sub Sub Category">
                            </div>
                        </form>
                    </div>
                </div>
...
@push('script')
    <script>
        $(document).ready( function () {
            //Category data show
            var sub_sub_category_dt = $('#sub_sub_category_dt').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    type: "post",
                    url: "{{ route('sub.sub.category.data.get') }}",
                    data: {
                        _token: _token,
                    },
                },
            ...
            $(document).on('click','.delete-data', function(e){
                e.preventDefault();
                var id = $(this).data('id');
                var url = "{{ route('sub.sub.category.delete') }}";
                Swal.fire({
                    title: 'Are you sure?',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                Swal.fire(
                                    'Deleted!',
                                    'Sub sub category has been deleted successfull.',
                                    'success'
                                )
                                sub_sub_category_dt.draw();
                            }
                        })
                    }
                })
